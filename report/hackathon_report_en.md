...
**FDR calibration depends on the null model.** A permutation null can be built two ways: **per-target** (each target's own permutation distribution, respecting its individual response variance) or **pooled** (all targets' permutation values concatenated into one shared reference distribution, which implicitly assumes every target is exchangeable). The two choices give materially different shortlists (`S1_fdr_audit.csv`):
